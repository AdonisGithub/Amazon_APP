...
class Business extends RuleTab {
    //add for debug
    getName() {
        return "Business";
    }
    constructor(props) {
        ...
        this.defaults = {
            ...this.defaults,
            business_rules: [], //saved business_rules
        };
    ...
    loadConfig() {
        ...
        let {business_rules} = Util.clone(config);
        let data_edit = Util.clone(this.defaults_edit);

        if (business_rules.length ) {
            business_rules = business_rules.filter(item => (item.hasOwnProperty('rules') && item.rules.name.length));
            data_edit.items  = Util.clone(business_rules);
    ...
    updateStatus = (event, id = -1) => {
        ...
        console.log("Business: updateStatus", this.data_edit.status, this.state, event, id);
            ...
            case States.EVENT_DELETE_DONE: {
                switch (preStatus) {
                    case States.STATE_EDIT:
                        ...
                        if (this.state.data.business_rules.length > 0) {
                            status = States.STATE_NORMAL;
                        } else {
                            status = States.STATE_EMPTY;
                        }
    ...
    doAdd = (data) => {
        let business_rule = {
            rules: BusinessForm.RULES,
            conditions: [BusinessForm.CONDITION],
            business_price: BusinessForm.BUSINESS_PRICE,
            quantity_discounts: [BusinessForm.QUANTITY_DISCOUNT],
        };
        let id = this.state.data.business_rules.length;
        data.business_rules[id] = Util.clone(business_rule);
        this.data_edit.items[id] = Util.clone(business_rule);
    ...
    doDelete = (data) => {
        ...
        const business_rules = this.state.data.business_rules.filter((item, i) => id !== i);
    ...
    doSave = (data) => {
        ...
        console.log("Business: doSave", id);

        data.business_rules[id] = Util.clone(this.data_edit.items[id]);
    ...
    doClose = (data) => {
        ...
        let id = this.data_edit.current;
        console.log("Business: doClose", id);

        id = parseInt(id);
        if( this.data_edit.edit[id] === States.EDIT_NEW ) {
            data.business_rules = this.state.data.business_rules.filter((item, i) => id !== i);
            this.data_edit.items = this.data_edit.items.filter((item, i) => (id !== i));
            this.data_edit.edit = this.data_edit.edit.filter((item, i) => id !== i);
        } else {
            this.data_edit.items[id] = Util.clone(this.state.data.business_rules[id]);
            this.data_edit.edit[id] = States.EDIT_SAVED;
        }
    }
    ...
    handlerEdit(id) {
        ...
        console.log("Business: handlerEdit", id);
    ...
    handlerFormSave = (cbSuccess, cbError) => {
        ...
        console.log("Business: handlerSave", this.data_edit.current, this.state);
        console.log(cbSuccess, cbError);

        const id = parseInt(this.data_edit.current);
        let business_rules = [];
        for (let i = 0; i < this.state.data.business_rules.length; i++) {
            if (i === id) {
                business_rules.push(this.data_edit.items[i]);
            } else {
                if (this.data_edit.edit[i] < States.EDIT_SAVED)
                    continue;

                business_rules.push(this.state.data.business_rules[i]);
            }
        }

        let configData = {...this.defaults, ...this.configurationGetCurrent(), business_rules};
    ...
    handleFormSimulate = (cbSuccess=null, cbError=null) => {
        console.log("Business: handleSimulate", this.data_edit.current);
        if (this.data_edit.current < 0) {
            return;
        }
        const id = parseInt(this.data_edit.current);
        let business_rule = Util.clone(this.data_edit.items[id]);
        business_rule.quantity_discounts.sort(this.compare);
        let configuration = this.shopify.getConfigurationSelected();
        let params = {configuration, operation: 'business_rules'};
        ApplicationApiCall.post('/application/rules/simulator', params, {business_rule: business_rule}, (json) => {
            if(cbSuccess){
                cbSuccess(json);
            }
        }, (err) => {
    ...
    handlerFormDelete = (cbError) => {
        ...
        console.log("Business: handlerSave", this.data_edit.current);
    ...
    handlerFormChange = (data) => {
        ...
        console.log("Business: handlerFormChange", this.data_edit.current);
    ...
    renderList() {
        const resourceName = {singular: 'business_rule', plural: 'business_rules'};

        // Let's consider if the rule has no name, this is a wrong rule and we remove it from the list
        // const items = this.state.data.business_rules.filter((item, i) => (this.data_edit.edit[i] !== States.EDIT_NEW && item.rules.name.length)); //error //@kbug_190131
        const status = this.data_edit.status;
        const items = this.state.data.business_rules;
        ...
        if (status === States.STATE_EMPTY) {
            let heading =
                <Stack>
                    <Stack.Item>
                        <Heading element="h3">{CsI18n.t("No business rule configured yet")}</Heading>
                    </Stack.Item>
                </Stack>
            ...
            return (
                <Card.Section>
                    ...
                    <Banner
                        status="default"
                        action={{content: <CsI18n>Add business rule</CsI18n>, onAction: this.handlerAddButton}}
                    >
                        {heading}
                        <p><CsI18n>A business rule is used to Business price and quantity discounts sent to Amazon</CsI18n></p>
                    </Banner>
                </Card.Section>
            );
        }
