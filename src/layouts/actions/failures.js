...
// import test_data from "../../testData/test_fail.json";
...
class FailuresTab extends AmazonTab {
  ...
  renderEmpty() {
    ...
    let message = 'Thanksfully, no failure has been recorded so far';
      ...
      <div>
        <ResourceList
          ...
          renderItem={(item, index) => {
            let {title, message, date_stop, logfile} = item;
            return (
              <ResourceList.Item
                id={index}
              >
                {index == 0 ?
                  <div className="list">
                    ...
                    <div className="batch_id"><Heading><CsI18n>Batch ID</CsI18n></Heading></div>
                    <div className="stop-date"><Heading><CsI18n>Date</CsI18n></Heading></div>
                  </div>
                  :
                  <div className="list">
                    <div className="title"><TextStyle>{title}</TextStyle></div>
                    <div className="message"><TextStyle>{message}</TextStyle></div>
                    <div className="batch_id"><Stack spacing={"extraTight"}><Stack.Item>{logfile}</Stack.Item><Stack.Item>{this.shopify.admin ?
                        <Stack.Item><Button plain
                                            onClick={this.handleDownloadBtnClick(logfile)}>
                          <span className={"download-button"}><DownloadIcon /></span>
                        </Button></Stack.Item>
                        : ""}</Stack.Item></Stack></div>
                    <div className="stop-date"><TextStyle>{date_stop}</TextStyle></div>
                  </div>}
              </ResourceList.Item>
            );
          }}
